...
if ( typeof url_socket_server == 'undefined' ) {
	var url_socket_server = "https://ontue.com:8443";
}
...
$(function(){
	...
	else trace("My room: " + $chat_room_name);
	...
	/** lobby chat-box */
	on_keypress( "#lobby-chat-box .chat-input [name='message']", socket_io_chat_send_message );
	$('body').on('click', '#lobby-chat-box  .chat-input .send', socket_io_chat_send_message );
	/** EO **/
...
function socket_io_chat_room_setting_for_room_join()
{
	...
	if ( ul ) socket_io_chat_room_setting( $chat_room_name, 'user allow', ul);
 ...
 */
function easyrtc_broadcast_room_whiteboard( draw, url )
{
...
{
	if ( ! $('#lobby-chat-box').length ) return;
	var box = get_lobby_chat_box();
	$('#lobby-chat-box').append( box );
}
function get_lobby_chat_box()
{
	return "<div class='chat-box'>" +
				"<div class='chat-message-wrapper'><div class='chat-message'></div></div>" +
				"<div class='chat-input'><input name='message' placeholder=' Input chat message'><span class='send cbutton'>SEND</span></div>" +
			"</div>"
}
...
{
	$('#video-chat-room').html("<div id='video-box'></div>");
}
/**
 *  화상 방 버튼을 추가한다.
 */
...
{	
	...
	if ( data.draw == 'begin' ) {
		...
		if ( data.mode == 'pen' ) {			
			...
			canvas_context.clearRect( data.x - 12, data.y - 12, 20, 20 );//this has different values from function canvas_context_clearRect
																		 //before which resulted to incorrect erase values
		}
	}	
...
function whiteboard_open( broadcast )
{
	var html = "<div id='white-board-wrapper'><div id='white-board'>" +
		...
		"</div></div>";
	$('#video-chat-room').append( html );
	
	if( is_observer == true ){//observer
		$( "#white-board" ).append("<div style='position:absolute;top:0;left:0; opacity:0; z-index:2300; height:800px; width:600px;'></div>");		
	}//observer
	 ...
	 */
	if ( typeof callback_whiteboard_open == 'function' ) callback_whiteboard_open( );
...
{
	var $input = $("#lobby-chat-box .chat-input [name='message']");
	var text = $input.val();
	if ( text == '' ) return;
	text = text.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;"); //to avoid php and js scripts on chatbox
...
function socket_io_chat_recv_message( data )
{
	...
	var $box = $("#lobby-chat-box .chat-message");
...
{
	trace("socket_io_chat_data_from_server:");
	trace_object(data);
	 ...
	 */
		...
		if ( typeof data.setting['whiteboard'] != 'undefined' ) {
			if ( data.setting['whiteboard'] == 'open' ) whiteboard_open();
		}
